...
def my_func():
    img_save_dir = str(FLAGS.vis_logdir)+"/images"
    color_save_dir = str(FLAGS.vis_logdir)+"/colored"
    raw_save_dir = str(FLAGS.vis_logdir)+"/raw_segmentation_results"

    tf.gfile.MakeDirs(img_save_dir)
    tf.gfile.MakeDirs(color_save_dir)
    tf.gfile.MakeDirs(raw_save_dir)

    return img_save_dir, color_save_dir, raw_save_dir
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
    ...
    for i in range(num_image):
        ...
        save_annotation.save_annotation(
            original_image, img_save_dir, image_filename,
            add_colormap=False)

        # Save prediction.
        save_annotation.save_annotation(
            crop_semantic_prediction, color_save_dir,
            image_filename, add_colormap=True,
