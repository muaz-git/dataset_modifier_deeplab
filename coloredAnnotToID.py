...
def get_gtFine_labelIds(color_annotated_img):
    ...
    for l in considered_labels:
        ...
        labelTrainIds_img[mask != 0] = l.id # changed from trainID to id because name of the file is a bit distracting.
    ...
    for l in not_considered_labels:
        if not (l.name == 'license plate') and not (l.name == 'polegroup'):
            ...
            # the mask
            mask = cv2.inRange(color_annotated_img, lower, upper)
            labelTrainIds_img[mask != 0] = 0  # ID for unlabeled
...
for i, s in enumerate(sample_annotation_files):
    ...
    labelTrainIds_img = get_gtFine_labelIds(color_annotated_img)
